    ...
    view = {
        ...
        editor : {
            init : function () {
                ...
                $('textarea:not(.noTinymce)').tinymce({
        ...
        */
        ui : {
            ...
            */
            ...
            tabs : {
                tabContainers : '#update_page_tabs',
                init : function () {
                    $(this.tabContainers).each(function (index, element) {
                        var elem = $(element),
                            tabBoxesSelector = [];

                        elem.find('li a').each(function (index, element) {
                            tabBoxesSelector.push(element.hash);
                            $(element).data('tabContainer', elem);
                        });

                        elem.data('tabBoxesSelector', tabBoxesSelector.join(','));
                        elem.delegate('a', 'click', view.ui.tabs.toggleTabs);
                    });
                },
                ...
                toggleTabs : function (e) {
                    ...
                    var tabContainer = $(this).data('tabContainer');

                    tabContainer.find('a').removeClass('active');

                    $(tabContainer.data('tabBoxesSelector')).addClass('hide');
            ...
            init : function () {
                ...
                $('.select_images_btn').click(function () {
                    var lang = $(this).attr('id').split('|')[1],
                        selectedImages = $('#multipleImages_list_' + lang).val().split(','),
                        i = 0;
                    ...
                    for (i = 0; i < selectedImages.length; i += 1) {
                       $('#listItem_' + selectedImages[i] + '_' + lang).addClass('selected');
                    }

                    $('#image_list_wrapper_'+ lang + ' ul:first li').click(function () {
                        $(this).toggleClass('selected');
                    });

                    $('#image_list_wrapper_'+ lang + ' button').one('click', function () {
                        var list = $(this).prev('ul'),
                            language = $(this).attr('lang'),
                            targetInput = $('#multipleImages_list_' + language),
                            selectedImagesIds = [];

                        list.find('li.selected').each(function () {
                            var id = $(this).attr('id').split('_')[1];

                            selectedImagesIds.push(id);
                        });

                        targetInput.val(selectedImagesIds.join(','));
                        
                        $('#image_list_wrapper_'+ language + ' ul:first li').unbind('click');
                        $('#image_list_wrapper_' + language).dialog('close')
                    });

                });

                $('#select_single_image_button').click(function () {
                    $('#select_single_image').dialog('option', 'title', 'Select single image').dialog('open');
                });

                $('.select_single_image_button').click(function () {
                    var language = $(this).attr('id').split('|')[1];
                    $('#select_single_image_' + language).dialog('option', 'title', 'Select single image').dialog('open');
                });

                $('#select_single_image a').click(function () {
                    var imageId = $(this).attr('rel'),
                        name = $(this).find('img').attr('alt');

                    $('#imageId').val(imageId);
                    $('#selected_image').text('Selected Image: ' + name);
                    $('#select_single_image').dialog('close');
                });
                ...
                $('.select_single_image a').click(function () {
                    ...
                    $('#imageId_' + language).val(imageId);
                    $('#selected_image_' + language).text('Selected Image: ' + name);
                    $('#select_single_image_' + language).dialog('close');
                });

                $('.image_list_wrapper, #select_single_image, .select_single_image').dialog({
                    autoOpen : false,
                    modal : true,
                    minHeight : 400,
                    minWidth: 940
        ...
        bridge: {
            ...
            */
            getValues: function() {
                ...
                $('#python_js_bridge input').each(function(index, elem) {
                    element = $(elem);
